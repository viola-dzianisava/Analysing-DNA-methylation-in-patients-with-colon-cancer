...
merged_data <- merge(gene_methylation_data, annot_df, by.x = "row.names", by.y = "Name", all.x = TRUE)

# Select and rename necessary columns using base R
tcga_columns <- grep("^TCGA", colnames(merged_data), value = TRUE)
coad_data <- merged_data[, c("Row.names", "chr", "pos", "UCSC_RefGene_Name", tcga_columns)]
...
coad_data <- coad_data[rowSums(is.na(coad_data[tcga_columns]) | coad_data[tcga_columns] == "") < length(tcga_columns), ]

# Display the final data
print(head(coad_data))

# Save the final data to a CSV file (optional)
write.csv(coad_data, "/Users/linxijun/Desktop/gene_methylation_data.csv", row.names = FALSE)
