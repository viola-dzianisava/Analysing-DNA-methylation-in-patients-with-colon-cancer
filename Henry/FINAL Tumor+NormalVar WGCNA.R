...
# Load the methylation data for the normal samples to get top 5000 genes
...
# Select top 5000 genes based on variance in the normal dataset
gene_variances_normal <- apply(methylation_values_clean_normal, 1, var)
top_genes_normal <- order(gene_variances_normal, decreasing = TRUE)[1:5000]
top_cpg_sites_normal <- cpg_sites_normal[top_genes_normal]
...
# Select the same genes (CpG sites) identified in the normal dataset
selected_genes_tumor <- methylation_values_clean_tumor[which(cpg_sites_tumor %in% top_cpg_sites_normal), ]

# Transpose the cleaned data for WGCNA
datMethyl_clean_tumor <- t(as.matrix(selected_genes_tumor))
...
output_dir <- "C:/Users/Henry/Desktop/Dissertation/FinalFindings/TN/"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

# Save the results for the tumor dataset
save_path_tumor <- "C:/Users/Henry/Desktop/Dissertation/FinalFindings/TN/WGCNA_results_tumor_top5000_normal.RData"
...
for (module in unique_modules_tumor) {
  ...
  heatmap_file <- paste0(output_dir, "heatmap_tumor_", module, ".png")
  tryCatch({
...
heatmap_tom_file <- paste0(output_dir, "WGCNA_heatmap_tumor.png")
heatmap_clusters_file <- paste0(output_dir, "WGCNA_clusters_heatmap_tumor.png")
